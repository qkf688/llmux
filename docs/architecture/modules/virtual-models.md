...
- **扩展点分布（现状）**：新增 `*_selector.go` + `RegisterSelector` 即可挂进分发；但现有 priority/round_robin/random 多为 **薄包装转发 `Service.selectBy*`**，新算法常仍要改 Service 方法；前端策略白名单（如 forms schema）需同步
