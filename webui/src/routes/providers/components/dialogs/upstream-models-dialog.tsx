        ...
        {Object.keys(upstreamTestResults).length > 0 && (
          <div className="flex items-center gap-2 text-xs text-muted-foreground flex-shrink-0">
            ...
            <span className="text-green-600">
              成功:{" "}
              {
                Object.values(upstreamTestResults).filter(
                  (r) => r.success === true,
                ).length
              }
            </span>
          ...
          <div className="flex items-center gap-3 px-3 py-2 bg-blue-50 border border-blue-200 rounded-md flex-shrink-0">
            <div className="flex-1">
              <div className="flex items-center justify-between text-xs text-blue-800 mb-1">
                <span>
                  测试进度：{upstreamBatchTestProgress.completed}/
                  {upstreamBatchTestProgress.total}
                  (成功: {upstreamBatchTestProgress.success}, 失败:{" "}
                  {upstreamBatchTestProgress.failed}, 进行中:{" "}
                  {upstreamBatchTestProgress.testing})
                </span>
                <span>
                  {Math.round(
                    (upstreamBatchTestProgress.completed /
                      upstreamBatchTestProgress.total) *
                      100,
                  )}
                  %
              ...
              <div className="w-full bg-blue-200 rounded-full h-2">
                <div
                  ...
                  style={{
                    width: `${(upstreamBatchTestProgress.completed / upstreamBatchTestProgress.total) * 100}%`,
                  }}
                />
              </div>
            </div>
        ...
        <div className="flex items-center justify-between gap-2 flex-shrink-0">
          ...
          <div className="flex gap-1 flex-wrap">
            ...
            <Tooltip>
              ...
              <TooltipContent>
                批量测试选中的 {selectedUpstreamModels.length} 个模型
              </TooltipContent>
            </Tooltip>
                        ...
                        <Tooltip>
                          <TooltipTrigger asChild>
                            <Button
                              ...
                              disabled={
                                !!testResult?.loading || upstreamBatchTesting
                              }
                            >
                              {testResult?.loading ? (
