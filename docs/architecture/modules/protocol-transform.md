...
- **三个 style 全覆盖，但 anthropic 的 DTO 只到顶层**：`service/anthropic/request_dto.go` 的 `anthropicRequest` 认领 13 个顶层键（model / stream / max_tokens / temperature / top_p / stop_sequences / metadata / system / messages / tools / thinking / output_config / tool_choice），嵌套结构一律停在 `json.RawMessage`，二次解析已分化：system / tools / thinking / output_config / tool_choice 走嵌套 struct DTO + 形状守卫（`anthropicThinking` / `anthropicOutputConfig` / `anthropicToolChoice` / `anthropicCacheControl` / `anthropicTextBlock` / `anthropicTool`），messages 的 content 块是**唯一**仍走 map 解析的字段（6 种 type 混排需自定义分派，理由见 `request_dto.go` 头注释）。**但对本检测而言两者等价**——检测只看顶层，嵌套 struct 化对它零贡献。**纪律**：`ClaimedRequestKeys` 反射的是 tag 而非「字段有没有被读」，往 DTO 加字段与把字段接进 `UnifiedRequest` 必须同一次改动，否则会把真实缺口伪装成已解决。anthropic 的 `top_k` / `service_tier` / `container` / `mcp_servers` 等当前确实未解析，被报出来是正确行为。
