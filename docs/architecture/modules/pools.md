...
```
handler/pools/           # 号池管理 API 域（S2：CRUD + 健康统计；S6 前端接入）
  routes.go              # Register：/api/pools GET/POST/PUT/:id/DELETE/:id
  crud.go                # GetPools(列表+统计) / CreatePool / UpdatePool / DeletePool
  types.go               # PoolRequest / PoolListItem(内嵌 models.Pool + 统计)
  repos.go               # repos() 访问 repository.Default()
repository/
  pool.go                # PoolRepo + StatsByIDs（GROUP BY 聚合健康统计）+ UpdateFields
  credential.go          # CredentialRepo + DeleteByPoolID（级联软删）+ UpdateFields
...
- 契约测试：`handler/pools/crud_test.go` 断言打在响应体 JSON（gjson `Exists()` 区分键存在与 null）；仓储测试在 `repository/channel_crud_test.go`
