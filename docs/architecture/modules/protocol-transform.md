...
- **Responses 出站 message item id 在 output 数组内唯一**：`service/responses/response_format.go` 的 message item 取 `msg_<unified.ID>`，多 choice 时第二个及之后追加 `_<choiceIdx>` 后缀。理由：`msg_` 是 Responses 的 **message 级**标识，客户端把 `response.output` 回灌成下一轮 `input` 是该协议的常规续话方式，重复 id 会被判 `Duplicate item found with id msg_...` 而 400。首个 choice **不加**后缀，保持单 choice（绝对主流形态）的 id 稳定。契约由 `TestFormatResponse_MultiChoiceMessageItemIDsAreUnique` + `format_response/multi_choice/openai-res.json` golden 锁住。注意 `function_call` item 的 id 是 `fc_<call_id>`，天然随 tool call 唯一，不受此规则影响

- **纯错误响应三协议各出自己的信封，守卫同口径**：`Error != nil && len(Choices) == 0` 时提前 return，openai / openai-res 出 `{"error":{message,type,code,param,request_id}}`，anthropic 出官方形状 `{"type":"error","error":{type,message},"request_id":...}`（`service/anthropic/response_outbound.go` 的 `formatErrorEnvelope`）。Anthropic 侧按官方契约裁剪：inner error 只保证 `type` / `message`，故统一模型的 `Code` / `Param` **刻意丢弃**（写出是非标准扩展键，拼进 `message` 又污染那段官方声明「可能变动、不可 pattern-match」的人类可读文案）；`request_id` 是**顶层**键且仅非空才写（空串会被读成「拿到了可用于联系支持的 ID」）；`error.type` 经 `mapAnthropicErrorType` 钳到官方 9 值枚举，未知/空值兜底 `api_error`——上游给的可能是 OpenAI 的 `rate_limit_exceeded` 这类本协议未定义值，原样透传等于给客户端一个无法分支的字符串。**刻意不按 HTTP 状态码反推 type**：`ResponseError.StatusCode` 在生产路径从不赋值（非 200 上游在 `chat_attempt` 就转重试，走不到出站格式化），按它推是凭空造数据。守卫的已知缺陷（`Error` 与 `Choices` 并存时错误被静默丢弃）三协议共有，未改。契约由 `TestFormatResponse_ErrorEnvelope*` + `format_response/error_only/anthropic.json` golden 锁住
...
- **budget 互转（`ReasoningEffortToThinkingBudget` / `shared.ThinkingBudgetToReasoningEffort`）**：6 档双向映射，**两个方向按归属分居两处**——正向 effort→budget 在 `service/anthropic/helpers.go`（取值编码的是 Anthropic 的 budget_tokens 硬地板，属 Anthropic 协议知识），反向 budget→effort 在 `service/transform/shared/thinking_budget.go`（输出的是 OpenAI effort 枚举、阈值来自 Octopus，协议中立，anthropic 入站与 openai 出站两个域都用）。正向：`minimal`/`low` → `MinThinkingBudget`(1024)，`medium`→20000，`high`→50000，`xhigh`→80000，`max`→128000。**低两档合并到 1024 是协议约束而非取舍**：Anthropic 明文拒绝 `budget_tokens < 1024`（此前 minimal→512、low→1000 出站必 400），而地板之下没有合法值可用来区分「更少」，故 minimal 与 low 对 Anthropic 上游不可区分（对 OpenAI 系上游无影响，那条路 effort 直传不换算）。反向区间保留 `1..512→minimal` / `513..19999→low`（描述的是客户端实际给了多小的 budget），因此低档 round-trip 非对称（512 → minimal → 1024），冻结在 `helpers_test.go`。正向「全档位返回 0 或 ≥ MinThinkingBudget」由 `TestReasoningEffortToThinkingBudget_AllTiersRespectMinBudget` 守卫——新增档位若给非法值该测试会红（用测试守而非在纯映射函数里加运行时钳制）。
...
- **openai 出站的 budget-only 降级 emit**：`transform/openai.FromUnified` 在 `ReasoningEffort == nil && ReasoningBudget != nil` 时 emit `reasoning_effort` = `shared.ThinkingBudgetToReasoningEffort(budget)`。原因：OpenAI chat-completions 协议**没有**任何 thinking token 数字段，budget 这一维无处可放；不降级则 budget-only 请求出站一个 thinking 字段都不带，上游按「不思考」处理且客户端收不到提示（比 400 更难发现）。**与上一条不矛盾**：上一条约束的是「不往 unified 写回 effort」（写回会让 `clampUnifiedReasoning` 把 budget-only 误判成 effort 请求、并给 responses 出站凭空加 `reasoning.effort`），本条只在**出站 body** 降级，unified 不动。顺序安全：钳制在 `FromUnified` 之前完成，降级用的 budget 已受白名单约束。有损（513..19999 全塌到 low）是协议固有代价。冻结在 `transform/thinking_intent_crossprotocol_test.go`（打在出站 body 上，responses 入站 → openai 出站端到端）。
- **分路径钳制**：
...
- **错误也进 golden**：`FromUnified` / `FormatResponse` 返错时包成 `{"__from_unified_error__": ...}` / `{"__format_response_error__": ...}` 冻结，而非 `Fatalf`。「某条路径当前就是返错」本身是覆盖矩阵要记录的现状；接线后 golden 从 error 变真实 body，diff 自证。**不**引入「哪个 style 期望 error」的期望表——那是 adapter 逻辑的第二份副本，必然漂移。
