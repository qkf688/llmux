...
import { getModelSyncLogs, deleteModelSyncLogs, clearModelSyncLogs, syncAllProviderModels, getModelSyncStats, getRecentAddedModels } from "@/lib/api";
import type { ModelSyncLog, ModelSyncStats, AddedModel } from "@/lib/api";
...
export default function ModelSyncLogsPage() {
  const [activeTab, setActiveTab] = useState<'logs' | 'recent'>('logs');
  ...
  const fetchRecentModels = useCallback(async () => {
    try {
      ...
      toast.error("加载最近新增模型失败: " + (err as Error).message);
    } finally {
            ...
            <>
              <div className="p-3 border-b bg-muted/30">
                <p className="text-sm text-muted-foreground">
                  最近同步时间: {formatDate(syncTime)} · 共新增 {recentModels.length} 个模型
                </p>
              </div>
              <div className="h-full overflow-auto">
                <div className="hidden sm:block w-full">
                  ...
                  {recentModels.map((model, idx) => (
                    <div key={idx} className="py-3 space-y-1">
                      ...
                      <div className="text-xs text-muted-foreground">
                        {model.provider_name} · {formatDate(model.added_at)}
                      </div>
                    </div>
                  ))}
                </div>
