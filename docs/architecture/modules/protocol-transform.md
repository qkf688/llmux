...
- **budget 互转（`ReasoningEffortToThinkingBudget` / `ThinkingBudgetToReasoningEffort`）**：6 档双向映射，single source of truth 在 `service/anthropic/helpers.go`。正向：`minimal`/`low` → `MinThinkingBudget`(1024)，`medium`→20000，`high`→50000，`xhigh`→80000，`max`→128000。**低两档合并到 1024 是协议约束而非取舍**：Anthropic 明文拒绝 `budget_tokens < 1024`（此前 minimal→512、low→1000 出站必 400），而地板之下没有合法值可用来区分「更少」，故 minimal 与 low 对 Anthropic 上游不可区分（对 OpenAI 系上游无影响，那条路 effort 直传不换算）。反向区间保留 `1..512→minimal` / `513..19999→low`（描述的是客户端实际给了多小的 budget），因此低档 round-trip 非对称（512 → minimal → 1024），冻结在 `helpers_test.go`。正向「全档位返回 0 或 ≥ MinThinkingBudget」由 `TestReasoningEffortToThinkingBudget_AllTiersRespectMinBudget` 守卫——新增档位若给非法值该测试会红（用测试守而非在纯映射函数里加运行时钳制）。
